...
const contractAddress = '0x4393250E747C6dcb99c781bc50F27CFba9951B8b';

// Scroll 链的 Chain ID（十六进制格式）
const SCROLL_CHAIN_ID = '0x82750'; // 请替换为实际的 Chain ID
...
const App = () => {
  ...
  const handleChainChanged = async (chainId) => {
    if (chainId !== SCROLL_CHAIN_ID) {
      alert('请切换到 Scroll 网络');
      try {
        await switchToScrollChain();
      } catch (error) {
        console.error(error);
        alert('切换网络失败，请手动切换到 Scroll 网络');
      }
      if (gameStarted) {
        setIsPaused(true);
      }
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        if (network.chainId !== parseInt(SCROLL_CHAIN_ID, 16)) {
          await switchToScrollChain();
        }
  ...
  return (
    <div className="game-board">
      ...
      {isPaused && <h2 className="paused">游戏已暂停，请切换回 Scroll 网络</h2>}
      <div className="board">
